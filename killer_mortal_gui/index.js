...
class GlobalState {
    ...
    refreshThemeColors() {
        ...
        this.C_colorText = style.getPropertyValue('--color-text').trim()
        this.C_colorSvgText = style.getPropertyValue('--color-svg-text').trim() || this.C_colorText
        this.C_colorSvgTextStroke = style.getPropertyValue('--color-svg-text-stroke').trim()
        this.C_colorBarMortal = style.getPropertyValue('--color-bar-mortal').trim()
        this.C_colorBarHero = style.getPropertyValue('--color-bar-hero').trim()
        this.C_colorTsumogiri = style.getPropertyValue('--color-tsumogiri').trim()
        this.C_colorTileBg = style.getPropertyValue('--color-tile-bg').trim()
        this.C_colorChoiceHighlight = style.getPropertyValue('--color-choice-highlight').trim() || 'hsl(0, 70%, 50%)'
        this.C_colorOpps = [style.getPropertyValue('--color-shimo').trim(),
                            style.getPropertyValue('--color-toimen').trim(),
                            style.getPropertyValue('--color-kami').trim()]
    }
    updateZoom() {
        if (document.documentElement.classList.contains('mortalcoach-embed')) {
            const widthScale = window.innerWidth / 1260
            const heightScale = window.innerHeight / 735
            this.C_zoom = Math.max(0.68, Math.min(1.02, Math.min(widthScale, heightScale)))
    ...
    updateCallBars() {
        ...
        for (let [idx, detail] of mortalEval.details.entries()) {
            ...
            if (mortalEval.actual_index == idx) {
                ...
                rect.setAttribute("stroke", GS.C_colorChoiceHighlight)
...
function createSvgText(x, y, text) {
    ...
    const embedded = document.documentElement.classList.contains('mortalcoach-embed')
    svg.setAttribute("fill", embedded ? GS.C_colorSvgText : GS.C_colorText)
    if (embedded) {
        svg.setAttribute("font-weight", "700")
        svg.setAttribute("font-size", "15")
        svg.setAttribute("paint-order", "stroke")
        svg.setAttribute("stroke", GS.C_colorSvgTextStroke)
...
function updateState() {
    ...
    notifyMortalCoachPosition()
    // discardOverflowTest()
}

function getCurrentMortalEvalPosition() {
    const hand = Number(GS.hand_counter)
    const ply = Number(GS.ply_counter)
    const round = GS.ge && GS.ge[hand]
    if (!round || !round[ply] || !round[ply].mortalEval) {
        return { hand, ply, kyoku_index: hand, has_mortal_eval: false }
    }
    let entryIndex = -1
    for (let idx = 0; idx <= ply; idx++) {
        if (round[idx]?.mortalEval) {
            entryIndex++
        }
    }
    return {
        ...
        has_mortal_eval: entryIndex >= 0,
    }
}

function notifyMortalCoachPosition() {
    if (!document.documentElement.classList.contains('mortalcoach-embed') || window.parent === window) {
        return
    }
    const position = getCurrentMortalEvalPosition()
    window.parent.postMessage({
        type: 'mortalcoach-position',
        ...position,
    }, '*')
}
...
function jumpToMortalEval(kyokuIndex, entryIndex) {
    kyokuIndex = Number(kyokuIndex)
    entryIndex = Number(entryIndex)
    if (!Number.isInteger(kyokuIndex) || !Number.isInteger(entryIndex)) {
        return { ok: false, reason: 'invalid-index' }
    }
    if (!GS.ge || kyokuIndex < 0 || kyokuIndex >= GS.ge.length) {
        return { ok: false, reason: 'kyoku-not-found' }
    }
    let seen = -1
    for (let ply = 0; ply < GS.ge[kyokuIndex].length; ply++) {
        const event = GS.ge[kyokuIndex][ply]
        if (!event.mortalEval) {
            continue
        }
        seen++
        if (seen == entryIndex) {
            GS.hand_counter = kyokuIndex
            GS.ply_counter = ply
            updateState()
            return { ok: true, hand: kyokuIndex, ply, entry_index: entryIndex }
        }
    }
    return { ok: false, reason: 'entry-not-found', seen }
}

function listMortalEvalPositions() {
    const positions = []
    if (!GS.ge) {
        return positions
    }
    for (let hand = 0; hand < GS.ge.length; hand++) {
        ...
        for (let ply = 0; ply < GS.ge[hand].length; ply++) {
            const event = GS.ge[hand][ply]
            if (!event.mortalEval) {
                continue
            }
            entryIndex++
            const mortalEval = event.mortalEval
            const details = mortalEval.details || []
            const actualIndex = mortalEval.actual_index
            const best = details[0] || {}
            const actual = details[actualIndex] || {}
            positions.push({
                hand,
                ply,
                kyoku_index: hand,
                entry_index: entryIndex,
                q_gap: Number(best.q_value || 0) - Number(actual.q_value || 0),
                actual_rank: Number(actualIndex || 0) + 1,
                is_equal: Boolean(mortalEval.is_equal),
            })
        }
    }
    return positions
...
function setMortalCoachTheme(theme) {
...
export default { main, GS, debugState, jumpToMortalEval, listMortalEvalPositions, fitToMortalCoach, setMortalCoachTheme } // So we can access these from dev console
function main() {
