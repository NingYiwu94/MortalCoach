...
3. 点击 `Download ZIP`。
4. 解压 ZIP。
5. 双击根目录里的 `Install-MortalCoach.bat`。
6. 安装完成后，双击桌面上的 `MortalCoach` 快捷方式启动。

`Install-MortalCoach.bat` 会自动检查并安装：
...
如果你不想使用安装脚本，可以先在 PowerShell 中运行：
...
安装完成后，直接双击桌面上的 `MortalCoach` 快捷方式。

如果没有桌面快捷方式，也可以双击根目录里的：
...
`Install-MortalCoach.bat` 使用 Windows 的 `winget` 安装 Python 和 Node.js。

大多数 Windows 10 / 11 都自带 winget。如果你的系统没有 winget，可以手动安装 Python 3.10+ 和 Node.js 20+，然后运行 `Start-MortalCoach.bat`。
...
```text
MortalCoach/
  Install-MortalCoach.bat     一键安装依赖并创建桌面快捷方式
  Start-MortalCoach.bat       启动入口
