...
1. 打开本仓库右侧的 `Releases`。
2. 下载最新版本里的 `MortalCoach-Setup-x.x.x.exe`。
3. 双击安装。
4. 安装完成后，双击桌面上的 `MortalCoach` 启动。

这种方式不需要你手动安装 Python、Node.js 或 Electron，适合普通玩家。

### 软件更新

安装包版本从 `v0.1.4` 开始支持软件内自动更新：

- 启动后自动检查 GitHub Release。
- 发现新版后，左侧会提示“发现新版本”。
- 点击“下载更新”后会在软件内下载更新包。
- 下载完成后点击“重启并更新”，软件会自动替换旧版本。

如果你当前使用的是 `v0.1.3` 或更早版本，需要先手动安装一次 `v0.1.4` 或更新版本。之后的版本就可以走软件内自动更新。本地牌谱库和训练档案会继续保留在 `%APPDATA%\MortalCoach\data\`。
...
3. 点击 `Download ZIP`。
4. 解压 ZIP。
5. 确认电脑已安装 Python 3.10+ 和 Node.js 20+。
6. 双击根目录里的 `Start-MortalCoach.bat`。

源码运行主要面向开发者和想自己改代码的用户。普通玩家优先使用安装包。

### 使用 Git

如果你会使用 Git，也可以这样运行源码版：
...
如果使用安装包，MortalCoach 会把个人数据保存在 Windows 用户目录：
...
使用安装包后，直接双击桌面上的 `MortalCoach` 快捷方式。
...
### 安装包还需要 Python 或 Node.js 吗？

不需要。`MortalCoach-Setup-x.x.x.exe` 已经包含桌面端和本地后端。

只有从源码运行时，才需要自己安装 Python 3.10+ 和 Node.js 20+。

### 支持 macOS / Linux 吗？

当前主要面向 Windows。核心后端是 Python + Web，理论上可以迁移，但桌面安装脚本和默认使用流程优先保证 Windows。

## 项目结构

```text
MortalCoach/
  Package-MortalCoach.bat     生成 Windows 安装包
  Install-MortalCoach.bat     源码版依赖安装脚本，备用
  Start-MortalCoach.bat       源码版启动入口
